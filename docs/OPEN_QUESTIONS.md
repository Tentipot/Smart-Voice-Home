...
30. Живата латентност 9–13 s: дали причината е препълнена VRAM, защото
    ...
    Отговорено диагностично на 2026-09-26: да (VRAM_RESIDENCY_20260926T153857Z).
    Остава изборът на решение и повторна жива проверка след него.

33. `run_stt_validation` отказва pilot_01: на 2026-09-26 13:37Z е приет втори
    BG01 запис (сесия 20260926T133731Z_…), а runner-ът изисква по една приета
    фраза. Кой BG01 да се ползва — решава потребителят.
...
34. Wake чрез CTC keyword spotting: фалшиви събуждания при сходни думи
    („Аврора“, „аура“, „оратор“), разговор и телевизор; честота за час;
    CPU (~0.6 s на проверка) или GPU (~0.02 s, +~0.6 GB VRAM); повторно
    използване на същите BG CTC резултати за езиковото решение.
