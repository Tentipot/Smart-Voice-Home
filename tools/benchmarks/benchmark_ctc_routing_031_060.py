...
FIRST_TEST = 31
LAST_TEST = 60
TEST_COUNT = LAST_TEST - FIRST_TEST + 1
...
def expected_track(
    ...
    raise ValueError(
        f"Test {number} is outside validation range."
    )
...
def route_from_entropy_delta(
    ...
    """
    Diagnostic rule defined before this hold-out corpus:

        BG minus EN entropy < 0  -> BG
        BG minus EN entropy > 0  -> EN

    Exact zero is treated as ambiguous.

    No threshold is tuned from tests 031-060.
    """
...
def main() -> None:
    ...
    print("=" * 88)
    print("CTC LANGUAGE ROUTING HOLD-OUT VALIDATION 031-060")
    print("=" * 88)
    print()
    print(
        "Routing rule was defined BEFORE this hold-out:"
    )
    print(
        "  entropy_delta = BG entropy - EN entropy"
    )
    print(
        "  entropy_delta < 0 -> BG"
    )
    ...
    print(
        "MIXED 051-060 are observed but NOT scored "
        "as BG/EN routing correctness."
    )
    print()
    ...
    print(
        f"BG loaded in {bg_load_seconds:.3f} s"
    )

    print(
        f"BG vocabulary: {len(bg_tokenizer)}"
    )
    ...
    print(
        f"EN loaded in {en_load_seconds:.3f} s"
    )

    print(
        f"EN vocabulary: {len(en_tokenizer)}"
    )
    ...
    for number in range(
        ...
        print(
            "DELTA min conf : "
            f"{deltas['min_confidence_delta']:+.4f}"
        )
    ...
    print()
    print("=" * 88)
    print("PURE LANGUAGE ROUTING SUMMARY")
    print("=" * 88)
    print()

    print(
        f"BG -> BG       : "
        f"{bg_correct}/{bg_total}"
    )

    print(
        f"EN -> EN       : "
        f"{en_correct}/{en_total}"
    )
    ...
    if pure_total > 0:
        print(
            f"PURE ACCURACY  : "
            f"{100.0 * pure_correct / pure_total:.1f}%"
        )

    print()
    print(
        "Previous Whisper AUTO resolver:"
    )
    print(
        "PURE CORRECT   : 12/20"
    )
    ...
    print()
    print("=" * 88)
    print("ALL ROUTE COUNTS")
    print("=" * 88)
    print()

    print(
        f"BG        : {route_counts['BG']}"
    )

    print(
        f"EN        : {route_counts['EN']}"
    )

    print(
        f"AMBIGUOUS : {route_counts['AMBIGUOUS']}"
    )

    print()
    print("MIXED 051-060 observations:")

    print(
        f"  routed BG        : "
        f"{mixed_route_counts['BG']}"
    )

    print(
        f"  routed EN        : "
        f"{mixed_route_counts['EN']}"
    )

    print(
        f"  routed AMBIGUOUS : "
        f"{mixed_route_counts['AMBIGUOUS']}"
    )

    print()
    print(
        "MIXED counts are descriptive only."
    )
    ...
    print(
        f"Combined avg / file   : "
        f"{total_ctc_inference / TEST_COUNT:.3f} s"
    )
    ...
    print()
    print("=" * 88)
    print("CTC LANGUAGE ROUTING VALIDATION COMPLETE")
    print("=" * 88)
    print()
