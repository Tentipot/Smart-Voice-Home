...
from app.speech.stt.whisper_engine import (
    WhisperSttEngine,
)
...
def main() -> None:
    ...
    print("=" * 72)
    print(
        "WHISPER / CTRANSLATE2 SAME-PROCESS "
        "GPU LIFECYCLE DIAGNOSTIC"
    )
    print("=" * 72)
    print()
    print(
    ...
    print()
    print(
        "Whisper uses faster-whisper / CTranslate2."
    )
    print(
        "PyTorch empty_cache() is measured only as "
        "a control step."
    )
    print()
    ...
    after_torch_cache_mb = print_measurement(
        "AFTER TORCH CACHE",
        baseline_mb=baseline_mb,
    )
    ...
    retained_after_torch_cache_mb = (
        after_torch_cache_mb - baseline_mb
    )
    ...
    released_after_torch_cache_mb = (
        loaded_mb - after_torch_cache_mb
    )
    ...
    print(
        f"Released after torch ctl : "
        f"{released_after_torch_cache_mb:+d} MB"
    )

    print(
        f"Retained after torch ctl : "
        f"{retained_after_torch_cache_mb:+d} MB"
    )
    ...
    print(
        ...
        "VRAM while Whisper is resident."
    )
    print(
        "- 'Retained after del + GC' is the important "
        "CTranslate2 lifecycle measurement."
    )
    print(
        "- torch.cuda.empty_cache() is only a control; "
        "it does not manage CTranslate2 allocations."
    )
    print(
        "- Small non-zero differences can come from "
        "runtime state or unrelated GPU activity."
    )
