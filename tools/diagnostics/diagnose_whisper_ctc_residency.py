...
def report_vram(label: str) -> None:
    print(
        f"{label}: VRAM used={gpu_used_mb()} MB",
        flush=True,
    )
...
def main() -> None:
    ...
    print(
        "=== WHISPER INT8_FLOAT16 + CTC GPU RESIDENCY ===",
        flush=True,
    )
    report_vram("Baseline")

    try:
        whisper_handle = run_measured(
            "Load Whisper",
            lambda: manager.acquire(
                ModelId.WHISPER_LARGE_V3
            ),
        )
        report_vram("After Whisper load")
        ...
        whisper = whisper_handle.resource
        ...
        print(
            f"CTC BG evidence: {ctc_bg!r}",
            flush=True,
        )

        whisper_en = run_measured(
            "Whisper EN short #1",
        ...
        print(
            f"Whisper EN text #1: {whisper_en.text!r}",
            flush=True,
        )
        ...
        print(
            f"CTC EN evidence: {ctc_en!r}",
            flush=True,
        )

        whisper_en_again = run_measured(
            "Whisper EN short #2",
            lambda: whisper.transcribe_file(
                EN_AUDIO,
                language="en",
        ...
        print(
            f"Whisper EN text #2: {whisper_en_again.text!r}",
            flush=True,
        )

        whisper_long = run_measured(
        ...
        print(
            f"Whisper 32s text: {whisper_long.text!r}",
            flush=True,
        )

        report_vram(
            "Both models resident after inference"
        )

    finally:
        print("=== CLEANUP ===", flush=True)

        # Remove references to resources before unloading.
        if ctc_handle is not None:
            ctc_handle.release()
            ctc_handle = None

        if whisper_handle is not None:
            whisper_handle.release()
            whisper_handle = None
